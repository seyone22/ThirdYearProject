  ...
  <div className='flex justify-between items-center text-base gap-8'>
    ...
    <ul className='md:flex space-x-12 '>
      {navItems.map(({ link, path }) => (
        <li key={path} className='inline-block'>
          <Link to={path} className='text-base text-black uppercase cursor-pointer hover:text-blue-700'>{link}</Link>
        </li>
      ))}
    </ul>
    ...
    <div className='md:hidden'>
      <button onClick={toggleMenu} className='text-black focus:outline-none'>
        {isMenuOpen ? <FaXmark className='h-5 w-5 text-black'/> : <FaBarsStaggered className='h-5 w-5 text-black hidden'/>}
      </button>
    </div>
  </div>
